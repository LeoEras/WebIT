...
def graficaProductividad(request, term_id, group_id, user_id):
    users = groupSelector(term_id, group_id)
    names = getNames(users, term_id)
    if int(user_id) not in users or int(user_id) == 0:
        user_id = users[0]
    all_dates = []
    user_activity = Log.objects.filter(termID=term_id)
    if int(term_id) == 1:
        ...
        user_activity = filterByDate(user_activity, "2017-07-01", "2017-08-10")
    list_1 = fetchAllDocInfofromUser(user_activity, term_id, user_id)
    list_1_prod = filterByRelevance(list_1, 1)
    list_1_unprod = filterByRelevance(list_1, 0)
    sorted_dates_1_prod, times_1_prod = buildDataset(list_1_prod)
    sorted_dates_1_unprod, times_1_unprod = buildDataset(list_1_unprod)
    all_dates = sorted_dates_1_prod + list(set(sorted_dates_1_unprod) - set(sorted_dates_1_prod))

    #Obteniendo los días de trabajo de los demás
    sorted_dates_1, times_1 = buildDataset(list_1)
    list_2 = fetchDocInfofromUser(user_activity, term_id, users[1])
    sorted_dates_2, times_2 = buildDataset(list_2)
    list_3 = fetchDocInfofromUser(user_activity, term_id, users[2])
    ...
    times_1_prod = depurateTimes(all_dates, sorted_dates_1_prod, times_1_prod)
    times_1_unprod = depurateTimes(all_dates, sorted_dates_1_unprod, times_1_unprod)
    prev_id, next_id = getNextPrevious(term_id, group_id)
    max_groups = maxGroups(term_id)
    prev_user_id, next_user_id = getNextPreviousUser(term_id, group_id, user_id)
    print(user_id)
    print(users)
    print(getNextPreviousUser(term_id, group_id, user_id))
    context = {'student': names[users.index(int(user_id))],
               'times_prod': times_1_prod,
               'times_unprod': times_1_unprod,
               'dates': all_dates,
               'group_id': group_id,
               'prev_id': prev_id,
               'next_id': next_id,
               ...
               'max_groups': max_groups}
    return render(request, 'tracker/graficaProductividad.html', context)
...
def getNextPreviousUser(term_id, group_id, current_user_id):
    users = groupSelector(term_id, group_id)
    index = users.index(int(current_user_id))
    if (index - 1) >= 0 and (index + 1) <= 3:
        return str(users[index - 1]), str(users[index + 1])
    elif (index - 1) < 0 and (index + 1) <= 3:
        return str(0), str(users[index + 1])
    elif (index - 1) >= 0 and (index + 1) > 3:
        return str(users[index - 1]), str(0)
